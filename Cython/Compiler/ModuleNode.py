...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def assure_safe_target(self, path, allow_failed=False):
        ...
        if not is_cython_generated_file(path, allow_failed=allow_failed, if_not_found=True):
            ...
            raise CompileError(
                self.pos,
                f'The output file already exists and does not look like it was generated by Cython: "{os.path.basename(path)}"',
            )
    ...
    def generate_h_code(self, env, options, result):
        ...
        if h_types or h_vars or h_funcs or h_extension_types:
            ...
            warning_string = EncodedString(
                f'Use PyImport_AppendInittab("{py2_mod_name}", {py3_mod_func_name}) instead of calling {py3_mod_func_name} directly.'
            )
            h_code_main.putln(
